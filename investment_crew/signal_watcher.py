...
[{{"index": 1, "ticker": "AAPL", "score": 75, "reason": "理由を1文で", "holding_period": "swing", "news_date": "ニュースの日付をそのまま記載"}}]
"""
...
def score_news_batch(news_items: list) -> list:
    ...
    # スコアリング後にレート制限リセットを待つ
    if result:
        print("  → レート制限リセット待機: 60秒...")
        time.sleep(60)
...
📰 材料（[ニュースの日付]）: [ニュースの核心を1〜2文でカジュアルに]
...
def run_deep_analysis(item: dict) -> str:
    ...
    news_date = item.get("news_date", "不明")
